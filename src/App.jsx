...
// Resets on every full page load; survives SPA navigation within the same session
...
function HomePage() {
  const [phase, setPhase] = useState(introShown ? 'site' : 'intro')
  ...
  return (
    <div style={{ background: 'var(--cream)', minHeight: '100dvh' }}>
      ...
      <AnimatePresence>
        {phase === 'intro' && (
          <IntroSequence onComplete={() => setPhase('burning')} />
        )}
      </AnimatePresence>

      <AnimatePresence>
        {phase === 'burning' && (
          <BurnTransition onComplete={() => setPhase('site')} />
        )}
      </AnimatePresence>
